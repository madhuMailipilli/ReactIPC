...
const VPAddAgent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    createUserMutation.mutate(userData, {
      ...
      onError: (error) => {
        ...
        const backendMessage = error.response?.data?.message || error.message || "Failed to create agent. Please try again.";
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
        ...
        <div className={`fixed top-6 right-6 z-[60] ${isClosing ? 'opacity-0 scale-95 transition-all duration-300' : 'animate-fadeIn'}`}>
          <div className="bg-white/90 backdrop-blur-md text-slate-900 px-5 py-4 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] flex items-center border border-slate-100 min-w-[320px]">
            ...
            <div className="flex-1">
              <p className="font-normal text-sm text-slate-900">Agent Created Successfully</p>
            </div>
            <button
      ...
      {showError && (
        <div className={`fixed top-4 right-6 z-[60] ${isClosing ? 'opacity-0 scale-95 transition-all duration-300' : 'animate-fadeIn'}`}>
          <div className="bg-white/90 backdrop-blur-md text-slate-900 px-5 py-4 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] flex items-start border border-rose-100 min-w-[320px] max-w-md">
            ...
            <div className="flex-1">
              <p className="font-normal text-sm text-slate-900">Error Creating Agent</p>
        ...
        <div className="p-6 relative">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Add New Agent</h1>
              </div>
            </div>
      ...
      <form id="add-user-form" onSubmit={handleSubmit} className="grid grid-cols-1 gap-6">
