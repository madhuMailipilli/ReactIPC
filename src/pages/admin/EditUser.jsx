  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.phones && formData.phones.length > 0) {
      updateData.phones = formData.phones
        ...
        .map((phone, index) => {
          const existingPhone = userData?.phones?.[index];
          return {
    ...
    if (formData.addresses && formData.addresses.length > 0) {
      updateData.addresses = formData.addresses
        ...
        .map((addr, index) => {
          const existingAddr = userData?.addresses?.[index];
          return {
    ...
    updateUserMutation.mutate(
      { id, data: updateData },
      {
        onSuccess: (response) => {
          setShowSuccess(true);
          setTimeout(() => {
            navigate(`/admin/user?page=${returnPage}`);
          }, 2000);
        },
        onError: (error) => {
          console.error('Error updating user:', error);
          const backendMessage = error.response?.data?.message || error.message || 'Failed to update user. Please try again.';
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        ...
        <span className="text-[10px] font-medium text-slate-400 uppercase tracking-widest animate-pulse">Loading Identity Matrix...</span>
      </div>
    );
  }
        ...
        <div className={`fixed top-4 right-6 z-[60] ${isClosing ? 'opacity-0 scale-95 transition-all duration-300' : 'animate-fadeIn'}`}>
          <div className="bg-white/90 backdrop-blur-md text-slate-900 px-5 py-4 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] flex items-center border border-slate-100 min-w-[320px]">
            ...
            <div className="flex-1">
              <p className="font-normal text-sm text-slate-900">Profile Updated</p>
              <p className="text-xs text-slate-500 font-normal">Agent changes saved successfully.</p>
            </div>
            <button 
      ...
      {showError && (
        <div className={`fixed top-4 right-6 z-[60] ${isClosing ? 'opacity-0 scale-95 transition-all duration-300' : 'animate-fadeIn'}`}>
          <div className="bg-white/90 backdrop-blur-md text-slate-900 px-5 py-4 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] flex items-start border border-rose-100 min-w-[320px] max-w-md">
            ...
            <div className="flex-1">
              <p className="font-normal text-sm text-slate-900">Error Updating User</p>
            ...
            <div className="p-6 space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  value={formData.role}
                  onChange={handleChange}
                  placeholder="e.g. Admin, User, VP"
                  className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-[13px] font-medium text-slate-700 placeholder:text-slate-500 focus:outline-none focus:ring-4 focus:ring-[#1B3C53]/5 focus:border-[#1B3C53] transition-all duration-300 hover:border-slate-400"
                />
              </div>
              
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 ml-1">Agency</label>
                <input
                  name="agency"
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 ml-1">Branch Code</label>
                <input
                  name="branchCode"
              ...
              <div className="flex items-center justify-between p-4 bg-white border border-slate-300 rounded-2xl hover:border-slate-400 transition-all">
                <div>
                  <div className="text-sm font-medium text-slate-700 mb-0.5">Status</div>
                  <div className="text-[13px] font-medium text-slate-900">{formData.active ? 'Active' : 'Inactive'}</div>
                </div>
