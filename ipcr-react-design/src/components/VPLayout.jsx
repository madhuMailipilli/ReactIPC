...
const VPLayout = () => {
  ...
  useEffect(() => {
    if (location.pathname.startsWith("/vp/agent")) {
      setExpandedMenus(prev => ({ ...prev, agents: true }));
    }
  }, [location.pathname]);
            ...
            <div key={groupIndex} className="mb-6">
              ...
              <div className="space-y-1">
                {group.items.map((item) => {
                  const isItemActive = item.path === "/vp" 
                    ? location.pathname === "/vp"
      ...
      <div className={`flex flex-col flex-1 min-w-0 h-screen transition-all duration-300 ${
        ...
        <header className="fixed top-0 right-0 left-0 h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 lg:px-8 xl:px-10 py-4 z-40">
          ...
          <div className="flex items-center gap-6 lg:gap-8">
            <Link to="/vp" className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex lg:flex flex-col items-end mr-1">
              ...
              <span className="text-xs font-medium text-slate-500" style={{ fontFamily: 'Poppins, sans-serif' }}>{user?.roles || 'VP'}</span>
            </div>
        ...
        {/* Main Content */}
        <main className="flex-1 overflow-auto bg-slate-50 p-3 mt-20">
